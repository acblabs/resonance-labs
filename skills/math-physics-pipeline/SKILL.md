...
*   **LLM Boundary**: A lab-assistant model may explain compact structured DSP summaries, experiment design, physics tutoring, troubleshooting, and evidence critique, but raw WAV and full high-dimensional grids must not be sent to the hosted path by default. LLM claim objects must cite JSON Pointer refs into the compact evidence packet; unresolvable refs should be rejected in favor of deterministic fallback text.
